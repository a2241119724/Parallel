...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        if attention_mask is not None:
            att[:, :, : , -attention_mask.shape[-1]:] = att[:, :, : , -attention_mask.shape[-1]:].masked_fill(attention_mask.bool(), -np.inf)
...
class ScaledDotProductAttentionMemory(nn.Module):
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        att = torch.matmul(q, k) / np.sqrt(self.d_k)  # (b_s, h, nq, nk)
        if attention_weights is not None: # None
            att = torch.cat([att[:, :, :, :nk] * attention_weights, att[:, :, :, nk:]], -1)
        if attention_mask is not None: # mask
...
class MultiHeadAttention(Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False,
        ...
        if attention_module is not None: # class
            if attention_module_kwargs is not None: # {'m': 40}
